<!--
**파일 역할**: Unreal Engine C++ 프로젝트의 범용 코딩 컨벤션 (모든 UE 프로젝트에서 재사용 가능)
**용도**: 네이밍 규칙, 메모리 관리, 네트워크 코딩, 최적화 등 UE 기본 규칙 정의
**프로젝트 특화**: 각 프로젝트는 이 파일을 상속받아 프로젝트별 규칙 추가 가능
**관련 파일**: Project/[ProjectName]/CODING_CONVENTIONS.md (프로젝트별 특화 규칙)
-->

# Unreal Engine C++ 코딩 컨벤션 가이드

이 문서는 Unreal Engine C++ 프로젝트에서 일관성 있는 코드 작성을 위한 범용 코딩 컨벤션을 정의합니다. 모든 규칙은 언리얼 엔진 공식 코딩 표준을 기반으로 합니다.
...
- **일반 로깅**: 프로젝트에서 정의한 로깅 매크로를 사용하여 일관성 있는 로그 출력을 유지합니다.
- **네트워크 로깅**: 네트워크 관련 로그는 별도의 로깅 매크로와 열거형을 사용하여 명확하게 구분합니다.
...
## 5. Doxygen 주석 스타일 가이드 (Doxygen Comment Style Guide)
